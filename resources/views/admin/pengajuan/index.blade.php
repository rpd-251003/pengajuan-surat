    ...
    <table class="table table-bordered table-striped mt-3">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <button class="btn btn-sm btn-info btn-cek-status"
                ...
                <td>
                    ...
                    @if($role == 'dosen_pa')
                        <form method="POST" action="{{ route('admin.pengajuan.approve_dosen_pa', $p->id) }}" class="d-inline">
                            @csrf @method('PATCH')
                            <button type="submit" class="btn btn-sm btn-success" onclick="return confirm('Yakin approve?')">Approve</button>
                        </form>
                        <button type="button"
                    ...
                    @elseif($role == 'kaprodi')
                        <form method="POST" action="{{ route('admin.pengajuan.approve_kaprodi', $p->id) }}" class="d-inline">
                            @csrf @method('PATCH')
                            <button type="submit" class="btn btn-sm btn-success" onclick="return confirm('Yakin approve?')">Approve</button>
                        </form>
                        <button type="button"
                    ...
                    @elseif($role == 'wadek1')
                        <form method="POST" action="{{ route('admin.pengajuan.approve_wadek1', $p->id) }}" class="d-inline">
                            @csrf @method('PATCH')
                            <button type="submit" class="btn btn-sm btn-success" onclick="return confirm('Yakin approve?')">Approve</button>
                        </form>
                        <button type="button"
                    ...
                    @elseif($role == 'staff_tu')
                        <form method="POST" action="{{ route('admin.pengajuan.approve_staff_tu', $p->id) }}" class="d-inline">
                            @csrf @method('PATCH')
                            <button type="submit" class="btn btn-sm btn-success" onclick="return confirm('Yakin approve?')">Approve</button>
                        </form>
                        <button type="button"
    ...
    document.querySelectorAll('.btn-cek-status').forEach(button => {
        button.addEventListener('click', function() {
            ...
            function makeStatusItem(levelName, approved_by, approved_at) {
                ...
                let badgeClass = isApproved ? 'bg-success' : 'bg-danger';
                let icon = isApproved ?
                ...
                return `<li class="list-group-item d-flex justify-content-between align-items-center">
                            ...
                            <span class="badge ${badgeClass} rounded-pill">
                                ${isApproved ? 'Disetujui' : 'Tidak Disetujui'} &nbsp; ${icon}
                            </span>
                        </li>`;
            }

            statusList.innerHTML += makeStatusItem('Dosen PA', data.dosen_pa.approved_by, data.dosen_pa.approved_at);
            statusList.innerHTML += makeStatusItem('Kaprodi', data.kaprodi.approved_by, data.kaprodi.approved_at);
            statusList.innerHTML += makeStatusItem('Wadek 1', data.wadek1.approved_by, data.wadek1.approved_at);
            statusList.innerHTML += makeStatusItem('Staff TU', data.staff_tu.approved_by, data.staff_tu.approved_at);
        });
    });
